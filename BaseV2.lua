...
local themes = {
	preset = {
		["accent"] = rgb(171, 136, 191),
		["high_contrast"] = rgb(5, 5, 5),
		["low_contrast"] = rgb(13, 13, 13),
		["inline"] = rgb(17, 17, 17),
		["outline"] = rgb(0, 0, 0),
...
if not isfile(library.directory .. "/fonts/fs-tahoma-8px.ttf") then
	writefile(library.directory .. "/fonts/fs-tahoma-8px.ttf", game:HttpGet("https://github.com/JUMPINGISACRIME/fonts/raw/refs/heads/main/fs-tahoma-8px.ttf"))
end
...
writefile(library.directory .. "/fonts/tahoma-font.json", http_service:JSONEncode(tahoma))
library.font = Font.new(getcustomasset(library.directory .. "/fonts/tahoma-font.json"), Enum.FontWeight.Regular)
...
function library:tween(obj, properties)
	local tween = tween_service:Create(obj, TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut, 0, false, 0), properties):Play()
	return tween
end
...
function library:window(properties)
	...
	local main_window = library:panel({
		...
		position = dim2(0, (camera.ViewportSize.X / 2) - 302, 0, (camera.ViewportSize.Y / 2) - 421 - 12),
	...
	task.spawn(function()
		while task.wait() do
			pcall(function()
				...
				table.insert(fpsValues, 1 / task.wait())
				if #fpsValues > 20 then
				...
				if currentTime - lastFpsUpdate >= fpsUpdateInterval then
					...
					watermark.change_text(string.format(
						properties.name .. " | %d FPS | %d ms | %d Players | %s",
						avgFps,
						ping,
						playerCount,
						os.date('%I:%M %p')
	...
	local holder = library:panel({
		name = "Configurations",
		size = dim2(0, 324, 0, 336),
		position = dim2(0, main_window.items.main_holder.AbsolutePosition.X + main_window.items.main_holder.AbsoluteSize.X + 2, 0, main_window.items.main_holder.AbsolutePosition.Y + 288 + 4),
	...
	section:button({
		name = "Teleport",
		callback = function()
			local player = game.Players:FindFirstChild(flags["PLAYERLIST_SELECTED"])
...
function library:notification(properties)
	...
	local watermark_outline = library:create("Frame", {
		...
		BorderColor3 = rgb(0, 0, 0),
		BorderSizePixel = 0,
		Position = UDim2.new(0, -300, 0, yPos),
	...
	task.spawn(function()
		...
		tween_service:Create(
			...
			{
				Position = UDim2.new(0, -300, 0, watermark_outline.Position.Y.Offset),
				BackgroundTransparency = 1
			}
		):Play()
		for _, v in next, watermark_outline:GetDescendants() do
			if v:IsA("TextLabel") then
				tween_service:Create(v, TweenInfo.new(0.4), {
					TextTransparency = 1
				}):Play()
	...
	local cfg = {
		name = options.name or "Section",
	       ...
	       local section = library:create("Frame", {
		       ...
		       Name = "\0",
		       BorderColor3 = rgb(0, 0, 0),
		       Size = options.size or dim2(1, 0, 1, 0),
		       BorderSizePixel = 0,
		       BackgroundColor3 = themes.preset.inline
	       })
	library:apply_theme(section, "inline", "BackgroundColor3")
...
function library:colorpicker(options)
	...
	section:button({
		...
		callback = function()
			...
			if library.copied_rainbow ~= nil then
				flags[cfg.flag .. "_RAINBOW_FLAG"] = library.copied_rainbow
			end
		end
	})
...
function library:keybind(options)
	...
	local cfg = {
		...
		open = false,
		binding = nil,
		name = options.name or self.name or nil,
	...
	if cfg.name and cfg.name ~= "Menu" then
	...
	local clicked_button = library:create("TextButton", {
		Parent = keybind_selector,
		Name = "",
		FontFace = library.font,
		TextColor3 = themes.preset.text,
	...
	clicked_button.MouseButton1Click:Connect(function()
		cfg.set_mode("click")
		cfg.set_visible(false)
		cfg.open = false
	end)
...
function library:playerlist(options)
	...
	local patterns = {
		["Priority"] = rgb(255, 255, 255),
		["Enemy"] = rgb(255, 0, 0),
		["Neutral"] = themes.preset.text,
		["Friendly"] = rgb(0, 255, 0)
	}
	...
	function cfg.create_player(player)
			       ...
			       pcall(function()
				       isFriend = lp:IsFriendsWith(players[tostring(player)].UserId)
			       end)
		...
		library:apply_theme(player_name, "accent", "TextColor3") 
		       local priority_text = library:create("TextLabel", {
			       ...
			       Name = "",
			       FontFace = library.font,
			       TextColor3 = tostring(player) ~= lp.Name and (isFriend and rgb(0, 255, 0) or themes.preset.text) or themes.preset.accent,
			       BorderColor3 = rgb(0, 0, 0),
			       Text = tostring(player) ~= lp.Name and (isFriend and "Friendly" or "Neutral") or "LocalPlayer",
			       BackgroundTransparency = 1,
			       TextXAlignment = Enum.TextXAlignment.Left,
			       BorderSizePixel = 0,
			       AutomaticSize = Enum.AutomaticSize.Y,
		...
		path.line = line
			   path.priority = isFriend and "Friendly" or "Neutral"
		path.priority_text = priority_text
		       TextButton.MouseButton1Click:Connect(function()
			       ...
			       flags["PLAYERLIST_SELECTED"] = player_name.Text
	...
	function cfg.search(text)
		...
		for _, player in next, players:GetPlayers() do
			...
			if path then
				...
				local sanity = lowerName:match(lowerText) or lowerDisplay:match(lowerText)
				path.instance.Visible = sanity and true or false
				path.line.Visible = sanity and true or false
			end
		end
	end
